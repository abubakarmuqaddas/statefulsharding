...
public class ShardedSNAPOptimizationReactivity {
    ...
    private HashMap<TrafficDemand, HashMap<Integer, HashMap<Vertex, IloNumVar>>> Y;
    boolean objectiveValueFixed;
    private double objectiveValue;
    private OptimizationOptions options;

    public ShardedSNAPOptimizationReactivity(ListGraph graph, TrafficStore trafficStore,
                ...
                for (Integer copy: Flows.get(trafficDemand).keySet()){
                    for (Vertex vertex : graph.getVertices()) {
                        if(!trafficDemand.getSource().equals(vertex)) {
                            Path path = ShortestPath.dijsktraCapacityConstrained(graph, trafficDemand.getSource(),
                                    vertex, trafficDemand.getDemand());
                            objective.addTerm(trafficDemand.getDemand() * path.getSize(),
                                    Y.get(trafficDemand).get(copy).get(vertex));
                        }
                    }
                }
            }
