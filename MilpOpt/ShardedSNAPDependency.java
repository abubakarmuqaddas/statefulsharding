...
public class ShardedSNAPDependency {
    ...
    private void buildModel(boolean outputRequired, boolean fixConstraints){
        ...
        try {
             ...
             *
             * P_{s_fn} + \sum_i P_{s_fuvin} \ge P_{t_gn}
             * \forall f \in F_s
             * \forall g \in F_t
             * \forall s_f,t_g \in c
             * \forall c \in C_{u,v}
    ...
    private void FixVariables() {
        ...
        try {
            ...
            for (Vertex vertex : graph.getVertices()) {
                for (StateVariable stateVariable : stateStore.getStateVariables()) {
                    for (StateCopy stateCopy : stateStore.getStateCopies(stateVariable)) {
                        if(stateCopy.getState().getLabel()=="A" && stateCopy.getCopyNumber()==1
                                && vertex.getLabel()==1){
                            cplex.addEq(Placement.get(vertex).get(stateCopy),1);
                        }
                        else if(stateCopy.getState().getLabel()=="A" && stateCopy.getCopyNumber()==2
                                && vertex.getLabel()==13){
                            cplex.addEq(Placement.get(vertex).get(stateCopy),1);
                        }
                        else if(stateCopy.getState().getLabel()=="B" && stateCopy.getCopyNumber()==1
                                && vertex.getLabel()==3){
                            cplex.addEq(Placement.get(vertex).get(stateCopy),1);
                        }
                        else if(stateCopy.getState().getLabel()=="B" && stateCopy.getCopyNumber()==2
                                && vertex.getLabel()==9){
                            cplex.addEq(Placement.get(vertex).get(stateCopy),1);
                        }
                        else{
            ...
            //List<StateCopy> A1B2 = getListStateCopy(StringAB,int12);
            //List<StateCopy> A2B1 = getListStateCopy(StringAB,int21);
            //List<StateCopy> A2B2 = getListStateCopy(StringAB,int22);
                ...
                for(List<StateCopy> stateCopies : Flows.get(trafficDemand).keySet()){
                    ...
                    for(Edge edge : Flows.get(trafficDemand).get(stateCopies).keySet()){
                        if(stateCopies.equals(A1B1) &&
                                edge.getSource().getLabel()==0 && edge.getDestination().getLabel()==1)
                            cplex.addEq(Flows.get(trafficDemand).get(stateCopies).get(edge),1.0);
                        else if(stateCopies.equals(A1B1) &&
                                edge.getSource().getLabel()==1 && edge.getDestination().getLabel()==2)
                            cplex.addEq(Flows.get(trafficDemand).get(stateCopies).get(edge),1.0);
                        else if(stateCopies.equals(A1B1) &&
                                edge.getSource().getLabel()==2 && edge.getDestination().getLabel()==3)
                            cplex.addEq(Flows.get(trafficDemand).get(stateCopies).get(edge),1.0);
                        else if(stateCopies.equals(A1B1) &&
                                edge.getSource().getLabel()==3 && edge.getDestination().getLabel()==7)
                            cplex.addEq(Flows.get(trafficDemand).get(stateCopies).get(edge),1.0);
                        else if(stateCopies.equals(A1B1) &&
                                edge.getSource().getLabel()==7 && edge.getDestination().getLabel()==11)
                            cplex.addEq(Flows.get(trafficDemand).get(stateCopies).get(edge),1.0);
                        else
